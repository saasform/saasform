...
var passport = require('passport-strategy')
  ...
  , MemoryStore = require('./state/memory')
  //, setup = require('./setup')
...
function Strategy(options, verify) {
  ...
  this._stateStore = options.store || new MemoryStore({ key: this._key });
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.code) {
        ...
        self._shouldLoadUserProfile(iss, sub, function(err, load) {
          ...
          function onProfileLoaded(profile) {
            ...
            if (self._passReqToCallback) {
              var arity = self._options.verifyArity || self._verify.length;
              if (arity == 9) {
    ...
    this._setup(identifier, req, function(err, config) {
      if (err) { return self.error(err); }
